...
public class AddVocabActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void saveDatabase(String id) {
        ...
        if (curTYPE == MOJI) {
            List<Moji> mListMoji = new ArrayList<>();
            ...
            List<Kanji> mListKanji = new ArrayList<>();
            int vocabCount = llData.getChildCount();
            if (vocabCount > 0) {
                for (int i = 0; i < vocabCount; i++) {
                    VocabView vocabView = (VocabView) llData.getChildAt(i);
    ...
    //save in local
    private void saveLocalDatabase(String id) {
